...
--------------------------------------------------------------------- CREATE
	...
	INSERT INTO Invoices (	InvoiceID,	VendorID,	InvoiceNumber,	InvoiceDate,	InvoiceTotal,
							PaymentTotal,			CreditTotal,	TermsID,		InvoiceDueDate,
							PaymentDate,			isDeleted)
		SELECT	[InvoiceID] = invoice.value('@id','int'),
				[VendorID] = invoice.value('@venId','int'),
				[InvoiceNumber] = invoice.value('@invNum','varchar(50)'),
				[InvoiceDate] = invoice.value('@invDate','smalldatetime'),
				[InvoiceTotal] = invoice.value('@invTot','money'),
				[PaymentTotal] = invoice.value('@paymentTot','money'),
				[CreditTotal] = invoice.value('@creditTot','money'),
				[TermsID] = invoice.value('@termsId','int'),
				[InvoiceDueDate] = invoice.value('@invDueDate','smalldatetime'),
				[PaymentDate] = invoice.value('@paymentDate','smalldatetime'),
				[isDeleted] = invoice.value('@isDeleted','bit')
		FROM @xml.nodes('AP-CRUD/CREATE/invoice') Invoice(invoice)
...
--------------------------------------------------------------------- READ
	...
	*/
	...
	SELECT	[id] = vendor.value('@id','int'),
			[name] = V.VendorName,
			[city] = V.VendorCity,
			[state] = V.VendorState,
			[comments] = CAST(vendor.query('comments/text()') AS VARCHAR(30)),
			recommendation = CAST(vendor.query('recommendation/text()') AS VARCHAR(50))
	FROM @xml.nodes('AP-CRUD/READ/vendor') Vendor(vendor)
		JOIN Vendors V ON vendor.value('@id','int') = V.VendorID
	ORDER BY V.VendorID
...
--------------------------------------------------------------------- UPDATE Terms
	...
	UPDATE Terms
		SET	TermsDescription = term.value('@desc','varchar(50)'),
			TermsDueDays = term.value('@days','smallint')
		FROM @xml.nodes('AP-CRUD/UPDATE/term') Term(term)
		WHERE term.value('@id','int') = TermsID

--------------------------------------------------------------------- UPDATE Vendors

	UPDATE Vendors
		SET	VendorName = vendor.value('@VendorName', 'varchar(50)')
		FROM @xml.nodes('AP-CRUD/UPDATE/vendor') Vendor(vendor)
		WHERE vendor.value('@id','int') = VendorID
...
*/
	...
	DELETE FROM InvoiceLineItems
		FROM @xml.nodes('AP-CRUD/DELETE/invoiceLineItems') Parent(parent)
		WHERE parent.value('@InvoiceID','int') = InvoiceID
