...
*/
    SELECT InvoiceNumber,DATEDIFF(day,InvoiceDate,PaymentDate) AS DaysPassed
    FROM Invoices
...
/*=========================================================================================== Q3 (5 rows):
     ...
     VendorID     TotalInvoices     InvoiceCount
        ...
        99           6940.25           1
*/
	DECLARE @page INT = 0, @records INT = 5

    SELECT VendorID, 
    SUM(InvoiceTotal)AS TotalInvoices,
    COUNT(InvoiceTotal)AS InvoiceCount
    FROM Invoices
    GROUP BY VendorID
    ORDER BY TotalInvoices DESC,InvoiceCount
...
*/
    ...
    SELECT VendorName, VendorPhone, VendorZipCode
    FROM Vendors
    WHERE VendorPhone LIKE '(800)%' AND VendorZipCode LIKE '[1-5][3-5]%5'
...
*/
     ...
     SELECT VendorName, 
            ...
            VendorPhone
     FROM Vendors
     WHERE VendorAddress1 LIKE '%Box%'AND VendorPhone IS NOT NULL
     ORDER BY VendorName
